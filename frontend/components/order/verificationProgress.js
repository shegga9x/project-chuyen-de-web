...
export default function VerificationProgress({ open, closeModal }) {
    ...
    const onXacMinh = async () => {
        if (isPDF) {
            ...
            console.log("ok");
            const res = await instance().post(`http://localhost:4000/api/digitalSignature/verifying`, formData).then(x => {
                setPDFResult(x.data);
                ...
                document.getElementById('pdfFail').style.display = 'block';
            })
        } else {
                    ...
                    <div id="pdfDiv" style={{ display: "none" }}>
                        <div className="verificationProgress_body_ele" style={{ display: "flex" }}>
                            ...
                            <input onChange={(event) => {
                                ...
                                document.getElementById('pdfSuccess').style.display = 'none';
                                document.getElementById('pdfFail').style.display = 'none';
                            }} id="pdfFileInput" type="file" style={{ display: "none" }}></input>
                            <button onClick={() => { document.getElementById('pdfFileInput').click() }}>Choose File</button>
                        </div>
                        <div id="pdfSuccess" style={{ display: "none" }}>
                            {
                                pdfResult != null && (
                                    <>
                                        <div className="verificationProgress_body_ele" style={{ display: "flex" }}>
                                            <p>Người  ký</p>
                                            <span>{pdfResult.name}</span>
                                        </div>
                                        <div className="verificationProgress_body_ele" style={{ display: "flex" }}>
                                            <p>Ngày Ký</p>
                                            <span>{pdfResult.signDate}</span>
                                        </div>
                                        <div className="verificationProgress_body_ele" style={{ display: "flex" }}>
                                            <p>Ngày hết hiệu lực</p>
                                            <span>{pdfResult.certificateInfo.notValidAfter}</span> </div>
                                    </>
                                )
                            }
                        </div>

                        <div id="pdfFail" className="verificationProgress_body_ele" style={{ display: "none" }}>
                            <p>File PDF sai định dạng hoặc đã bị chỉnh sửa</p>
                        </div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                        <button onClick={() => { typeChange() }} style={{ outline: 'none', border: 'none', backgroundColor: 'orangered', marginRight: "10px" }}>
                            {isPDF ? (<>Quay Lại</>) : (<>Chuyển Sang PDF</>)}</button>
                        <button onClick={() => { onXacMinh() }} style={{ outline: 'none', border: 'none', backgroundColor: 'orangered', marginRight: "10px" }}>Xác minh</button>
                        <button onClick={() => { closeModal('verificationProgress') }} style={{ outline: 'none', border: 'none', backgroundColor: 'orangered' }}>Thoát</button>
                    </div>
                </div>
            </div>
        </>
