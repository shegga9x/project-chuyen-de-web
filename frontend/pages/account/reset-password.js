...
export default function ResetPassword({ err, token }) {
    ...
    const checkSubmit = async () => {
        ...
        if (newPassword != confirmNewPassword) {
            ...
            const res = await axios.post('http://localhost:4000/api/accounts/reset-password', { token: tk, password: newPassword, confirmPassword: confirmNewPassword })
            if (res) {
...
export async function getServerSideProps({ query }) {
    let err1 = "";
    const res = await axios.post('http://localhost:4000/api/accounts/validate-reset-token', { token: query.token })
        .catch(err => {
