...
export default function Profile({ keyword, customer }) {
    ...
    useEffect(() => {
        const changeInformationCustomer = async () => {
            const res = await instance().post("https://localhost:4000/api/customer/changeInformationCustomer", customerState)
                .catch(err => {
...
export async function getServerSideProps({ req, query }) {
    ...
    if (session) {
        const res = await instance({ req }).get("https://localhost:4000/api/customer/getCurrentCustomer")
