...
export default function Profile() {
    ...
    const formik = useFormik({
        ...
        validationSchema,
        onSubmit: values => {
            InstanceAxios().post(`https://localhost:4000/api/saler/updateShop`, values).then((response) => {
    ...
    useEffect(() => {
        async function fetchMyAPI() {
            if (session) {
                ...
                await InstanceAxios().get(`https://localhost:4000/api/saler/getShopProfile`).then((data) => {
