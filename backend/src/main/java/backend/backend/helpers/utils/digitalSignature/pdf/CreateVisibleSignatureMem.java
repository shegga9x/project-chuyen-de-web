 ...
 * pattern" influenced
 * PDVisibleSignDesigner, and doesn't create its complex multilevel forms
 * described in the Adobe
 * document
 * <a href=
...
public class CreateVisibleSignatureMem extends CreateSignatureBase {
     ...
     * @throws IOException
     */
    public Calendar signPDF(InputStream inputStream, OutputStream signedStream, String tsaUrl,
    ...
    public static byte[] generateQRcode(String data, int h, int w) throws IOException, WriterException {
        ...
        // MatrixToImageWriter.writeToFile(matrix, path.substring(path.lastIndexOf('.')
        // + 1), new File(path));
    ...
    private InputStream createVisualSignatureTemplate(PDDocument srcDoc, int pageNum, PDRectangle rect)
        ...
        try (PDDocument doc = new PDDocument()) {
            ...
            InputStream fontInputStream = CreateVisibleSignatureMem.class.getClassLoader()
                    .getResourceAsStream("digitalSignature/calibri.ttf");
    ...
    private static void addHeader(PDPageContentStream cs, float w, float h, PDFont font) throws IOException {
        ...
        cs.showText("Chứng nhận tài liệu kỹ thuật số");
        cs.endText();
    ...
    private static void addFooter(PDPageContentStream cs, float w, float h, PDDocument srcDoc) throws IOException {
        ...
        cs.showText("Trang 1 của " + srcDoc.getNumberOfPages());
        cs.endText();
    }

    private static void addCenterPart(PDPageContentStream cs, float w, float h, PDFont font, Calendar signDate)
            throws IOException {

        cs.beginText();
        cs.setFont(font, 7);
        cs.newLineAtOffset(w / 2 - 40, h - 40);
        cs.showText("Được xác minh bởi Shop");
        cs.newLine();
    ...
    private static void addCenterOverlay(PDPageContentStream cs, float w, float h, PDDocument doc, byte[] imageBytes)
            throws IOException {
    ...
    private static void addLeftPart(PDDocument doc, String qrcode, String uuid, PDPageContentStream cs, PDFont font,
        ...
        cs.showText("Bạn có thể kiểm tra tính hợp lệ của tài liệu");
        cs.newLine();
        cs.showText("quét mã QR hoặc nhập mã");
        cs.newLine();
        cs.showText("tại https://www.hcg.gr/validate");
        cs.newLine();
        cs.newLine();

        cs.setFont(font, 9);
        cs.showText("Mã tài liệu:");
        cs.newLine();
        cs.showText(uuid);
        cs.endText();
    ...
    private static void addRightPart(PDPageContentStream cs, PDFont font, float w, float h, Calendar signDate,
            String visibleLine1, String visibleLine2) throws IOException {
        float fontSize = 9f;
        cs.setFont(font, fontSize);
        showTextRight(cs, font, "Ký kết bởi:", w, h - 40, fontSize);
        showTextRight(cs, font, visibleLine1, w, h - 50, fontSize);
        showTextRight(cs, font, visibleLine2, w, h - 60, fontSize);
        showTextRight(cs, font, "Ngày ký:", w, h - 70, fontSize);
        SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
        showTextRight(cs, font, sdf.format(signDate.getTime()), w, h - 80, fontSize);
    }

    private static void showTextRight(PDPageContentStream cs, PDFont font, String text, float w, float y,
